<?php require_once './head.php' ?>
<?php require_once './nav.php' ?>

<main>
  <section id="reference" class="py3">
    <div class="banner phuket-3">
      <h1 class="heading-2 pt3">Pool Villas</h1>
    </div>

    <div class="container container-narrow text-center pt2">
      <p class="pb1">
      We are offering for sale 3 Bedrooms Pool Villa which are all fully furnished, fully fitted kitchen, and luxurious package furniture by Kenkoon. 
      </p>
      <p class="pb1">
      We are surrounded with the best architects and interior designers to develop the concept of our resort hotel. We also have the support of The KENKOON BRAND, a contemporary furniture brand that skillfully uses teak wood, marble and steel.
      </p>
      <p class>
        Please see the picture gallery below showing the layout and design of our pool villas.
      </p>
    </div>

    <div class="container pt1">

      <div class="gallery">

        <figure class="gallery-item">
          <img src="images/16-9-opt/ED_1.jpg" alt="Pool Villa 3D layout from above" />
          <figcaption class="gallery-caption">
            <p>Pool Villa 1 layout</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/ED_1.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa Schematic 1"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/ED_002.jpg" alt="Pool Villa 3D layout from above" />
          <figcaption class="gallery-caption">
            <p>Pool Villa 2 layout</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/ED_002.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa Schematic 2"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN01.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN01.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN001.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN001.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN02.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN02.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN002.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN002.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN03.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN03.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN003.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN003.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN04.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN04.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

        <figure class="gallery-item">
          <img src="images/16-9-opt/FN004.jpg" alt="Pool Villa" />
          <figcaption class="gallery-caption">
            <p>Pool Villa</p>
            <a class="btn btn-small" 
               href="images/16-9-opt/FN004.jpg"
               data-lightbox="pool-villa"
               data-title="Pool Villa"
              >View
            </a>
          </figcaption>
        </figure>

      </div>
    </div>
  </section>
</main>

<?php require_once './footer.php' ?>

</body>
</html>
